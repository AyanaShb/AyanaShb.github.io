...
function getP(){
    ...
    if (!tipe) {
        ...
        document.getElementById('imagex').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'line://app/1653379581-X9kDOMeB?type=image&img=https://www.crackbot.us/images/1540203011314-1.jpg\n\ntype => image\nimg => Link (must be HTTPS)'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('videox').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'line://app/1653379581-X9kDOMeB?type=video&ocu=https://goo.gl/5bJq6M&piu=https://www.crackbot.us/images/1540203011314-1.jpg\n\ntype => video\nocu => video url\npiu => preview image'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('audiox').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'line://app/1653379581-X9kDOMeB?type=audio&link=https://moepoi.xyz/audio/soundtrack?file=QcgmXm8kg__mTCESSzPZSw.opus'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        ...
        document.getElementById('profilex').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'line://app/1653379581-X9kDOMeB?type=profile'
            }]).then(function () {
                liff.closeWindow();
            });
        });
                    ...
                    actions: [
                        {
                            type:"uri",
                            label:"Me",
                            uri:"line://app/1653379581-X9kDOMeB?type=profile"
                        }
                    ]
                }
            }]).then(function () {
